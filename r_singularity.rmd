...
x <- readRDS(gzcon(url("https://bwlewis.github.io/r-and-singularity/chr21-22-pca.rdata",open="rb")))
library(lattice)
splom(x$v)
```

We see some obvious clusters in the data, but the clusters are not all that
well-defined because we only use partial data from two chromosomes (21 and 22)
...
Let's assume for this example that we have 4 total computers. Then we need to invoke
...
```
```{r, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
x <- readRDS(gzcon(url("https://bwlewis.github.io/r-and-singularity/1239-680-r4-4xlargex4-chr1-22-pca.rdata", open="rb")))
library(lattice)
splom(x$v)
```

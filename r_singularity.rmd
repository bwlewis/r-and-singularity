...
scientists to spend time on system administration and less time on their
problems.
...
applications shows that there is a huge demand for lightweight, rapidly
deployable containers. Notably, these technologies have not been widely
successful in HPC settings. Shifter (https://github.com/NERSC/shifter) is the
most successful attempt to use Docker in HPC, but suffers from several
...
We can run the `%test` script with
```
...
```
mpirun -np 4 singularity test container.img
```

...
- Assumes that the input variant files are split up among working directories
...
LANG=C CHUNKSIZE=10000000 mpirun -np 2 singularity run -H $(pwd) variant_pca.img 
```
...
When running on more than one computer, first distribute the vcf.gz files to
working directories on each computer. Each computer will only process the files
located in its working directory.

Replace `-np 2` with the number of computers available in your cluster plus
one. That isOr, submit the job using an available cluster job manager like slurm.
